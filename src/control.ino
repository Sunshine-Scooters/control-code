/*
   ...
   Adapted from code written by FIRST Team 1939 (The Kuhnigits) to utilize Cytron PS2 Shield as parent override device.
...
const boolean DISABLE_BACKWARDS   = false;   // Whether or not to disable backward travel for child
...
// Constants
const int SPEED_LIMIT = 512;        // Between 0-512
const int SPEED_MINIMUM = 80;       // Between 0-SPEED_LIMIT
const int TURN_SPEED_MINIMUM = 80;  // Between 0-128 - Minimum turn speed
const int REVERSE_SPEED_MINIMUM = 80; // Between 0 and SPEED_MINIMUM
const int FORWARD_SPEED_MINIMUM = 80; // Between 0 and SPEED_MINIMUM
...
void loop() {
  ...
  // CHILD MODE CONTROL CODE X/Y position setting
    ...
    // Invert y due to orientation of new joystick mount
    if (y != 512) {
      y = map(y, 0, 1023, 1023, 0);
    }
    ...
    */
    ...
    // Invert x due to orientation of new joystick mount
    if (x != 512) {
      x = map(x, 0, 1023, 1023, 0);
    }
    ...
    */
    ...
    /*
      Control scheme V2 - if the Y value is above 512, then center the X. This has the
      effect of ignoring L/R input and creates the pure forward triangle zone over the left and right.
    */
    if (y > 512 || y < 512) {
      x = 512;
    }
  ...
  */
  ...
  if (y < 512) {
    // Dampen the reverse by taking a portion of the distance below 512
    y = 512 - ( (512 - y) / REVERSE_FACTOR);

    // Set the minimum reverse speed
    y = min(y, 512 - REVERSE_SPEED_MINIMUM);
  }
  // Otherwise if we're going forward ...
  else if (y > 512) {
    // Set the minimum forward speed
    y = max(y, 512 + FORWARD_SPEED_MINIMUM);
  }
